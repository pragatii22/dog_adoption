...
def open_register():
    def register_user():
        ...
        if not(username and password and address and email and phone_no):
            ...
            return
        try:
            cursor.execute("INSERT INTO users VALUES(?,?,?,?,?)",(username, password,email, address,phone_no))
            conn.commit()
    ...
    lbl_password.pack()
    entry_password=tk.Entry(reg_win, width=35,font=("Arial",11))
    entry_password.pack()
...
def open_change_password(username):
        ...
        if result:
            cursor.execute("UPDATE users SET password=? WHERE username=?", (entry_new_password, username))
            conn.commit()
